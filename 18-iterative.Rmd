...
To practice this use case, let's use the dataset [`humpback-whales.csv`](https://github.com/databrew/intro-to-data-science/blob/main/data/humpback-whales.csv?raw=true): 15 years of sightings of humpback whales within a fjord system in northern British Columbia, Canada ([www.bcwhales.org](www.bcwhales.org)). 
...
Each row shows a whale (column `id`) identified during a single sghting (column `sit`). Some sightings were of a single solo individual, but others were of groups of two or more individuals.   
...
To perform a correlation analysis with randomization, let's use a classic scenario from marine conversation: We have abundance estimates for a critically endangered porpoise from four years across two decades of research. This species only has a handful of individuals left in the wild. ([link to dataset](https://github.com/databrew/intro-to-data-science/blob/main/data/porpoise-survey.csv?raw=true))
...
Using these genetic samples, the researchers determined the relatedness of each pair of individuals they sampled. Those results are provided in the dataset [`humpback-kinship.rds`](https://github.com/databrew/intro-to-data-science/blob/main/data/humpback-kinship.rds?raw=true).  

```{r,echo=TRUE}
kinship <- readRDS("./data/humpback-kinship.rds")
```

And [here is a key](https://github.com/databrew/intro-to-data-science/blob/main/data/bnf-status.rds?raw=true) providing the IDs of these 36 whales and their status as a known bubble net feeder.
...
For your convenience, the social association rate for these 36 individuals has also been summarized in the dataset [`humpback-sociality.rds`](https://github.com/databrew/intro-to-data-science/blob/main/data/humpback-sociality.rds?raw=true).  
