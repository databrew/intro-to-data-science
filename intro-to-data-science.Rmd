...
**Step 5.** Write a function that takes any name and plots its proportional prevalence from 1880 to 2017. Format the plot beautifully. Provide the name as the title of the plot.  
...
**Step 8.** Create a function called `first_letter`. This should take any vector as an argument and return the first letter only. You will need to use the `substr` function (look it up).

**Step 9.** Use your `first_letter` function to make a new variable in the babynames dataset called `fl`. This should be the first letter of all names.

**Step 10.** What was the most popular first letter of boys names in 1900?

**Step 11.** What was the least popular first letter of girls names in 2017?

**Step 12.** Make a function called `letter_plot`. This should take two arguments, `letter` and `m_or_f`, and then create a plot showing the popularity of that letter for the letter/sex combination inputted over time.

**Step 13.** Make a function called `letter_compare`. This should take two  arguments: `y` (the year) and `return_table` (logical/boolean, whether or not to return a table). If `return_table` is `FALSE`, the function should return a plot. Otherwise, a table. In either case, it should show the popularity of all letters for the year in question.
...
Let' start by reading in some data
...
1. How may rows in the data? Whhhaaatttt?
