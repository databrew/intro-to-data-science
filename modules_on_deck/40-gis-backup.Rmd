...
Geograpchi = location=based. everything is tied to a location.

Information = database

System = table and geography are tied together
...
Most data are geographic
And for most datasets, there are important questions to be asked in terms of geography

Three types of vector data:
building blocks of most GIS

Points 0D
Line 1D
Polygon (connected lines, there's an inside and an outside (Start and end point are the same)) 2D

Vector data are stored in shapefiles
ESRI invented the shapefile

Raster data: photos / images
really good for data that are everywhere -- elevation is everywhere, temperature is everywhere

vectors are made for discrete objects
rasters are best for continuous phenomena
...
Visual: what is where?
Overlay: what is on what?
Proximity: what is near what?
Statistical: what is the distribution



@TerribleMaps

We don't often read scatterplots in life, but we read maps ALL the time.  
