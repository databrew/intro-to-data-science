...
\section{\texorpdfstring{\texttt{read.csv()}}{read.csv()}}\label{read.csv}}

\hypertarget{readrread_csv}{%
\section{\texorpdfstring{\texttt{readr::read\_csv()}}{readr::read\_csv()}}\label{readrread_csv}}
...
A dataframe has rows of data organized into columns. Let's use a dataset on fuel mileage for all cars sold from 1985 to 2014.
...
Each row pertains to a single vehicle make/model -- i.e., a single \emph{observation}. Each column pertains to a single \emph{type} of data. Columns are named in the \emph{header} of the dataframe.
...
\hypertarget{part-accessing-managing-data}{%
\part{Accessing \& managing data}\label{part-accessing-managing-data}}
...
\section*{Learning goals}\label{learning-goals-19}}
\addcontentsline{toc}{section}{Learning goals}
...
To practice this use case, let's use the dataset \texttt{humpback-whales.csv}: 15 years of sightings of humpback whales within a fjord system in northern British Columbia, Canada (\url{www.bcwhales.org}).
...
This indicates that the chance that the slope is lower than the null expectation is 87.7\%.
...
This indicates that the observed regression is better than 74.2\% of scenarios that might be expected under the null expectation.
...
Using these genetic samples, the researchers determined the relatedness of each pair of individuals they sampled. Those results are provided in the dataset \texttt{humpback-kinship.rds}.
...
For your convenience, the social association rate for these 36 individuals has also been summarized in the dataset \texttt{humpback-sociality.rds}.
