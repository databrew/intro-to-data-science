...
We won't go into statistical theory here. But every statistical test, at the basic level, is asking whether the patterns we observe in our data are actually *meaningful.* Are those *observed* patterns reflecting *real* patterns, or is it possible to observe those patterns when the underlying process is purely random chance? 

That is what a **p-value** tells you: the probability that the pattern you see in your data is actually the product of random, meaningless chance. The lower the p-value, the less likely it is that the pattern you are seeing is a product of chance, and the higher the chances that the pattern you are seeing is real. 
...
Finally, please also note that statistical significance is a tricky thing because it is influence by *both* the patterns in your data as well as by the *amount* of data you are working with. A larger sample size is going to allow you to identify more nuanced patterns in your data, and those patterns will achieve a lower p-value thanks simply to the number of samples you have. Conversely, a pattern driven by meaningful real-world processes can be obscured by low sample sizes. If your sample size is too small, you may not be able to produce significant p-values even though the pattern is real. 
...
**1. Categorical data** represent categories. For example, franchise ID above. As another example, let `pet` be a variable representing categorical data with a few discrete levels: dog, cat, etc. The types, or *levels*, of categorical data are also referred to as *treatments*. In `R`, categorical variables are usually referred to as *factors*.

**2. Continuous data** represent numerical values. For example, the weight of French fries above. As another example, let `height` be a variable representing numerical values for people's height. Note that, for the time being, we will consider integers and count data to fall within this category.  
...
**Scenario:** You are the lead author on a landmark study that investigates the portion sizes in fast food restaurants. You place 50 orders of large fries from one franchise and 50 orders from another. 
...
Let's pretend we don't know what the *true* means of these two sample sets are. Let's pretend that all we have done is weighed and eaten a ton of french fries, and know we want to ask some research questions about our sample data. 
...
barplot(means,
        names.arg=c("Franchise 1","Franchise 2","Franchise 3"),
        ylim=c(0,1.2),col="darkblue")
```
...
saltiness <- -0.25*sample1 + .6 + variability
```
