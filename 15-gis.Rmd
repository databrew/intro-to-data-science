...
Subset data by year and indicator

```{r,echo=TRUE,collapse=TRUE, eval = TRUE}
df <- df %>% filter(indicator_name =='Mean BMI, male (age 18+)')
...
# map text
map_palette <- colorNumeric(palette = brewer.pal(9, "Greens"), domain=world_shp@data$value, na.color="#CECECE")


leaflet(world_shp, options = leafletOptions(minZoom = 1, 
                                                  maxZoom = 10)) %>% 
...
# Create map
map_text <- paste(
  "Indicator: ",  world_shp@data$indicator_name,"<br>",
  "Economy: ", as.character(world_shp@data$country),"<br/>", 
  'Value: ', round(world_shp@data$value, digits = 2),  "<br/>",
  "Year: ", as.character(world_shp@data$year),"<br/>",sep="") %>%
